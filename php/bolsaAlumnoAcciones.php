...
if (!class_exists('MySQL')) { require_once $_SERVER["DOCUMENT_ROOT"].'/php/conex.php'; }
	...
	$idWork2 = "";
		...
		$sqlNTB .= "AND B.id NOT IN (SELECT bolsaId FROM AlumnoBolsa WHERE AlumnoId = ".$_SESSION["personaIdForWorks"]." ) ";
...
echo "<td align=center><span class='listCantPost'>".$value['postulados']."</span></td>";
							...
							echo "<div class='btnCuadrado aceptar' onclick='postularWork(".$value['id'].")' title='Autorizar'><img alt='' src='imagenes/iconos/check_blanco.png' width='15'></div>";
				...
				<script>
function postularWork(id) {
	var confirmar = confirm("Confirme que desea postularse a la publicacion");
	if (confirmar == true) {
		$('#mibolsa').load('php/bolsaAlumnoAcciones.php', {accion : 'post', id: id });
	}
	else {
		return (false);
	}
